...
export class VehiclesService {
  ...
  async getVehicles() {
    return this.prisma.vehicle.findMany({ where: { isActive: true }, orderBy: { level: 'asc' } });
  }

  async purchaseVehicle(userId: string, vehicleId: string) {
    const vehicle = await this.prisma.vehicle.findUnique({ where: { id: vehicleId } });
    if (!vehicle || !vehicle.isActive) throw new NotFoundException('Vehicle not found');

    await this.wallet.deductCoins(userId, vehicle.coinPrice, `Purchase vehicle: ${vehicle.name}`, vehicleId);
  ...
  async activateVehicle(userId: string, vehicleId: string) {
    ...
    if (!userVehicle) throw new NotFoundException('You do not own this vehicle');
    if (userVehicle.expiresAt && new Date() > userVehicle.expiresAt) {
      throw new BadRequestException('Vehicle has expired');
    }

    await this.prisma.userVehicle.updateMany({ where: { userId, isActive: true }, data: { isActive: false } });
