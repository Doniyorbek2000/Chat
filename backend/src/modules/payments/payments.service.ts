...
import { InitiatePaymentDto, ClickPrepareDto, ClickCompleteDto, PaymeDto, PaymentProvider } from './dto/payments.dto';
...
export class PaymentsService {
  ...
  async initiatePayment(userId: string, dto: InitiatePaymentDto) {
    ...
    const transaction = await this.prisma.transaction.create({
      data: {
        ...
        metadata: { provider: dto.provider, packageId: dto.packageId, amount: dto.amount },
      },
    });
  ...
  async handleClickPrepare(dto: ClickPrepareDto) {
    ...
    if (transaction.status !== TransactionStatus.PENDING) return { error: -4, error_note: 'Already paid' };
  ...
  async handleClickComplete(dto: ClickCompleteDto) {
    ...
    if (!transaction || transaction.status !== TransactionStatus.PENDING) {
      return { error: -4, error_note: 'Transaction not found or already processed' };
    }

    await this._completePurchase(transaction.id, transaction.userId);
    return { error: 0, error_note: 'Success', click_trans_id: dto.click_trans_id, merchant_trans_id: dto.merchant_trans_id };
  }
  ...
  async handlePayme(dto: PaymeDto) {
    ...
    switch (dto.method) {
      case 'CheckPerformTransaction': {
        ...
        if (!transaction) return { error: { code: -31050, message: { en: 'Order not found' } }, id };
      ...
      case 'CreateTransaction': {
        ...
        if (!transaction) return { error: { code: -31050, message: { en: 'Order not found' } }, id };
        return {
          result: { create_time: Date.now(), transaction: transaction.id, state: 1 },
          id,
        };
      }
      ...
      case 'PerformTransaction': {
        ...
        const transaction = await this.prisma.transaction.findFirst({
          where: { metadata: { path: ['paymeTransId'], equals: transactionId } },
        });
        if (!transaction) return { error: { code: -31003, message: { en: 'Transaction not found' } }, id };

        await this._completePurchase(transaction.id, transaction.userId);
        return { result: { transaction: transaction.id, perform_time: Date.now(), state: 2 }, id };
      }
      ...
      case 'CancelTransaction': {
        ...
        return { result: { transaction: transactionId, cancel_time: Date.now(), state: -1 }, id };
      }

      default:
        return { error: { code: -32601, message: { en: 'Method not found' } }, id };
    }
  }
  ...
  async getTransactionHistory(userId: string, page = 1, limit = 20) {
    ...
    const [items, total] = await Promise.all([
      ...
      this.prisma.transaction.count({ where: { userId, type: TransactionType.RECHARGE } }),
    ]);
  ...
  private async _completePurchase(transactionId: string, userId: string) {
    ...
    await Promise.all([
      this.walletService.processRecharge(userId, meta?.amount || 0, pkg.coins, transactionId),
