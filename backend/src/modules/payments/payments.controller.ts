...
import { InitiatePaymentDto, ClickPrepareDto, ClickCompleteDto, PaymeDto, GetTransactionHistoryDto } from './dto/payments.dto';
...
export class PaymentsController {
  ...
  @Get('history')
  async getHistory(@CurrentUser() user: any, @Query() query: GetTransactionHistoryDto) {
    const data = await this.paymentsService.getTransactionHistory(user.id, query.page, query.limit);
    return { success: true, data };
  }

  @Post('google-play/verify')
  async verifyGooglePlay(@CurrentUser() user: any, @Body() dto: GooglePlayVerifyDto) {
